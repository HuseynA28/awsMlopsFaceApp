...
def genderapp():
    if request.method == 'POST':
       ...
       # Make sure the image name has a valid extension (e.g., .jpg or .png)
            gray_image_name = f'roi_{i}.jpg'  # Ensure it has a valid extension
            eig_image_name = f'eigen_{i}.jpg'  # Ensure it has a valid extension

            # Save the images with the correct extension
            matimg.imsave(os.path.join('./static/predict', gray_image_name), gray_image, cmap='gray')
            matimg.imsave(os.path.join('./static/predict', eig_image_name), eigen_image, cmap='gray')
